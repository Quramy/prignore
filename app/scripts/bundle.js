...
function decorate(opt) {
  var editorUrl = '/' + [opt.group, opt.repo, 'edit', opt.branchName, '.prignore'].join('/');

  _jquery2['default'].ajax({ editorUrl: editorUrl }).then(function (data) {
    var content = (0, _jquery2['default'])(data).find('textarea[data-filename=".prignore"]').text();
    var ignore = _gitignoreParser2['default'].compile(content);
